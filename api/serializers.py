...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'email', 'first_name', 'last_name', 'phone_number', 'user_type', 'password','national_id', 'kra_pin', 'next_of_kin_name', ]
        read_only_fields = ['national_id', 'kra_pin','next_of_kin_id']
        ...
        if user_type == "member":
            if not data.get("national_id"):
                raise serializers.ValidationError(
                    {"national_id": "This field is required for members."}
                )
            if not data.get("next_of_kin_name"):
                raise serializers.ValidationError(
                    {"next_of_kin_name": "This field is required for members."}
                )

        elif user_type == "manager":
            if not data.get("email"):
                raise serializers.ValidationError(
                    {"email": "This field is required for managers."}
                )
        else:
...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def validate_email(self, value):
        ...
        cache.set(
            f"otp_{user.id}",
            {"code": otp_code, "expires_at": timezone.now() + timedelta(minutes=10)},
            timeout=600,
        )
        send_mail(
